...
export class LanGameReplayError extends Error {
  ...
  readonly code = 'LAN_GAME_REPLAY_INVALID' as const
 ...
 */
export function replayMatchEvents(events: readonly MatchEvent[], asOfSeq?: number): MatchReplay {
  const first = events[0]
  const last = events.at(-1)
  if (first === undefined || last === undefined) throw new LanGameReplayError('match event log is empty')
  ...
  let state: DoudizhuState | undefined
  let round = 0
  let deal = 0
  let totalScores: [number, number, number] = [0, 0, 0]
  const roundResults: DoudizhuResult[] = []
  let decisionOutcomes: MatchCheckpoint['decisionOutcomes'] = []
    ...
    try {
      switch (event.type) {
        ...
        case 'decision-requested': {
          ...
          if (pendingDecision !== undefined) throw new Error('a second decision was requested while one is pending')
        ...
        case 'decision-abandoned':
          ...
          if (pendingDecision?.requestId === event.requestId) pendingDecision = undefined
          break
        ...
        case 'round-finished': {
          const current = requireState(state)
          if (current.phase !== 'finished' || current.result === undefined) throw new Error('round finished before engine settlement')
 ...
 */
export function validateMatchCheckpoint(
 ...
 * @returns Human-readable invariant failures, or an empty array when valid.
 */
export function validateMatchRecord(record: MatchRecord, roomId = record.room.id): string[] {
  ...
  if (record.closedAt !== undefined) {
    if (record.pendingDecision !== undefined) failures.push('closed record must not retain pendingDecision')
...
function assertSequence(events: readonly MatchEvent[]): void {
  let previous: MatchEvent | undefined
  for (const event of events) {
...
function requireState(state: DoudizhuState | undefined): DoudizhuState {
  if (state === undefined) throw new Error('event requires an earlier deal-started event')
  return state
}

function decisionSeat(state: DoudizhuState): 0 | 1 | 2 {
