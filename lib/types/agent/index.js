...
import { LanGameAgentError } from "./error.js";
import { LAN_GAME_ACTION_TOOL, } from "./types.js";
...
export default class LanGameAgents extends Service {
    ...
    async decide(request) {
        ...
        const result = new Promise((resolve, reject) => {
            const onAbort = () => {
                if (entry.pending?.requestId !== request.requestId)
                    return;
        ...
        this.publish(entry);
        try {
            await this.ctx.subagents.followup(request.parent, request.childId, [{ type: 'text', text: decisionPrompt(request.requestId, request.stateVersion, request.state) }], {
    ...
    setupChild(childCtx) {
        ...
        if (child === undefined || !String(child.id).startsWith(CHILD_PREFIX))
            return () => { };
        ...
        const disposeTool = childCtx.tools.register(defineTool({
            ...
            execute: (args, exec) => {
                if (exec.agent !== child)
                    throw new LanGameAgentError('decision tool caller does not match its Game Session', 'LAN_GAME_PARENT_MISMATCH');
                ...
                const decision = { requestId: args.requestId, stateVersion: args.stateVersion, action: args.action };
                pending.cleanup();
                entry.pending = undefined;
        ...
        const disposePreStep = childCtx.on('agent/pre-step', async ({ messages }, next) => {
            ...
            const pending = entry.pending;
            if (pending !== undefined && messages.every(message => matchesPendingDecision(message, pending)))
                return next();
            return { kind: 'reject' };
        });
        return () => {
    ...
    entry(childId, parent) {
        ...
        if (entry === undefined)
            throw new LanGameAgentError(`Game Session ${JSON.stringify(childId)} does not exist`, 'LAN_GAME_NOT_FOUND');
        if (entry.parent !== parent)
            throw new LanGameAgentError('foreground parent does not own this Game Session', 'LAN_GAME_PARENT_MISMATCH');
        return entry;
    }
...
function replaceDecisionHistory(agent) {
    ...
    if (only?.type === 'user/message'
        ...
        && only.data.source.plugin === 'dsh-doudizhu/agent')
        return;
...
function matchesPendingDecision(message, pending) {
    if (message.source.kind !== 'coordinator')
        return false;
