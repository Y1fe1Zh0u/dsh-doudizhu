...
import { DOUDIZHU_RANKS, doudizhuCard, doudizhuRankIndex, sortDoudizhuCards, } from "./cards.js";
/**
 ...
 */
export function classifyDoudizhuCombination(input) {
    ...
    if (groups.length === 1) {
        ...
        if (cards.length === 1)
            return combination('single', cards, rank, 1);
        if (cards.length === 2 && rank !== 'SJ' && rank !== 'BJ')
            return combination('pair', cards, rank, 1);
        if (cards.length === 3)
            return combination('triple', cards, rank, 1);
        if (cards.length === 4)
            return combination('bomb', cards, rank, 1);
    }
    if (cards.length === 4 && counts.join(',') === '3,1')
        return bodyCombination('triple-single', cards, groups, 3);
    if (cards.length === 5 && counts.join(',') === '3,2')
        return bodyCombination('triple-pair', cards, groups, 3);
    ...
    for (const [kind, mode, unit] of [
        ...
        if (primary !== undefined)
            return combination(kind, cards, primary, chainLength);
    }
    ...
    if (cards.length === 8 && counts.join(',') === '4,2,2')
        return bodyCombination('four-two-pair', cards, groups, 4);
    return null;
}
/**
 ...
 */
export function beatsDoudizhuCombination(challenger, current) {
    if (challenger.kind === 'rocket')
        return current.kind !== 'rocket';
    if (current.kind === 'rocket')
        return false;
    if (challenger.kind === 'bomb' && current.kind !== 'bomb')
        return true;
    if (current.kind === 'bomb' && challenger.kind !== 'bomb')
        return false;
 ...
 */
export function legalDoudizhuPlayActions(hand, current) {
    ...
    const add = (cards) => {
        ...
        if (classified === null)
            return;
        if (current !== undefined && !beatsDoudizhuCombination(classified, current.combination))
            return;
        plays.set(sorted.join('\u0000'), { action: { type: 'play', cards: sorted }, combination: classified });
    };
    for (const group of groups) {
        add(group.cards.slice(0, 1));
        if (group.cards.length >= 2 && !isJoker(group.rank))
            add(group.cards.slice(0, 2));
        if (group.cards.length >= 3)
            add(group.cards.slice(0, 3));
    ...
    for (const body of groups.filter(group => group.cards.length >= 3)) {
        for (const wing of groups.filter(group => group.rank !== body.rank)) {
            ...
            if (wing.cards.length >= 2 && !isJoker(wing.rank))
                add([...body.cards.slice(0, 3), ...wing.cards.slice(0, 2)]);
        }
    }
    ...
    for (const body of consecutiveRuns(groups.filter(group => group.cards.length >= 3), 2)) {
        ...
        for (const wings of choose(singleWings, body.length)) {
            if (!containsRocket(wings))
                add([...core, ...wings.map(group => required(group.cards[0], 'airplane wing'))]);
        }
    ...
    for (const body of groups.filter(group => group.cards.length === 4)) {
        ...
        for (const pair of choose(pairWings, 2))
            add([...body.cards, ...pair.flatMap(group => group.cards.slice(0, 2))]);
    }
...
function consecutive(groups, minimum) {
    if (groups.length < minimum || groups.some(group => doudizhuRankIndex(group.rank) > doudizhuRankIndex('A')))
        return false;
...
function airplanePrimary(groups, chainLength, mode) {
    ...
    for (const body of bodyCandidates) {
        ...
        if (mode === 'none' && remainder.length === 0)
            valid.push(primary);
        if (mode === 'single' && remainder.length === chainLength) {
            const wingGroups = rankCounts(remainder);
            if (wingGroups.every(([, count]) => count === 1) && !containsRocketRanks(remainder))
                valid.push(primary);
        }
...
function consecutiveRuns(groups, minimum) {
    ...
    for (const group of eligible) {
        if (current.length === 0
            || doudizhuRankIndex(group.rank) === doudizhuRankIndex(required(current.at(-1), 'current run tail').rank) + 1)
            current.push(group);
        else {
    ...
    for (const run of maximal) {
        for (let length = minimum; length <= run.length; length += 1) {
            for (let start = 0; start + length <= run.length; start += 1)
                result.push(run.slice(start, start + length));
        }
    }
    return result;
}
