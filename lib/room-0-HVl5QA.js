...
var LanRoom = class LanRoom {
	...
	*/
	static restore(snapshot) {
		...
		if (failures.length > 0) throw new LanRoomError(`cannot restore invalid room snapshot: ${failures.join("; ")}`, "LAN_ROOM_INVALID_ARGUMENT");
	...
	*/
	join(memberId) {
		...
		if (this.members.some((member) => member.id === memberId)) throw new LanRoomError(`member ${JSON.stringify(memberId)} already joined room ${JSON.stringify(this.id)}`, "LAN_ROOM_MEMBER_EXISTS");
		if (this.members.length >= this.maxMembers) throw new LanRoomError(`room ${JSON.stringify(this.id)} already has ${this.maxMembers} members`, "LAN_ROOM_FULL");
		this.members.push({
			id: memberId,
	...
	*/
	updatePrompt(request) {
		...
		this.requirePhase("lobby");
		if (!PROMPT_HASH.test(request.promptHash)) throw new LanRoomError("promptHash must be a lowercase SHA-256 hex digest", "LAN_ROOM_INVALID_ARGUMENT");
	...
	*/
	setReady(request) {
		...
		this.requirePhase("lobby");
		const member = this.member(request.memberId);
		if (request.ready && member.promptHash === void 0) throw new LanRoomError(`member ${JSON.stringify(member.id)} must set a prompt before becoming ready`, "LAN_ROOM_PROMPT_REQUIRED");
		if (request.ready && !member.connected) throw new LanRoomError(`member ${JSON.stringify(member.id)} must be connected before becoming ready`, "LAN_ROOM_INVALID_ARGUMENT");
		member.ready = request.ready;
		if (this.members.length === this.maxMembers && this.members.every((candidate) => candidate.ready && candidate.connected)) this.phase = "locked";
		return this.commit();
	}
	/**
	...
	*/
	finish(request) {
		...
		if (result.length === 0 || result.length > 256) throw new LanRoomError("result must contain 1 to 256 characters", "LAN_ROOM_INVALID_ARGUMENT");
		this.result = result;
		this.phase = "finished";
		return this.commit();
	}
	...
	*/
	leave(memberId, expectedRevision) {
		...
		this.requirePhase("lobby");
		if (memberId === this.coordinatorId) throw new LanRoomError("the coordinator closes the room through the owning service", "LAN_ROOM_NOT_COORDINATOR");
	...
	member(memberId) {
		...
		if (found === void 0) throw new LanRoomError(`member ${JSON.stringify(memberId)} is not in room ${JSON.stringify(this.id)}`, "LAN_ROOM_MEMBER_NOT_FOUND");
		return found;
	}
	requireRevision(expected) {
		if (!Number.isSafeInteger(expected) || expected < 0) throw new LanRoomError("expectedRevision must be a non-negative safe integer", "LAN_ROOM_INVALID_ARGUMENT");
		if (expected !== this.revision) throw new LanRoomError(`room ${JSON.stringify(this.id)} is at revision ${this.revision}, not ${expected}`, "LAN_ROOM_STALE_REVISION");
	}
	requirePhase(expected) {
		if (this.phase !== expected) throw new LanRoomError(`room ${JSON.stringify(this.id)} is ${this.phase}, expected ${expected}`, "LAN_ROOM_INVALID_PHASE");
	}
	requireCoordinator(memberId) {
		if (memberId !== this.coordinatorId) throw new LanRoomError(`member ${JSON.stringify(memberId)} is not the room coordinator`, "LAN_ROOM_NOT_COORDINATOR");
	}
	commit() {
		this.revision += 1;
...
*/
function validateLanRoomSnapshot(snapshot) {
	const failures = [];
	...
	for (const member of snapshot.members) {
		...
		if (member.promptHash !== void 0 && !PROMPT_HASH.test(member.promptHash)) failures.push(`member ${JSON.stringify(member.id)} has an invalid prompt hash`);
		if (member.ready && member.promptHash === void 0) failures.push(`ready member ${JSON.stringify(member.id)} has no prompt hash`);
	}
	if (snapshot.phase !== "lobby" && (snapshot.members.length !== snapshot.maxMembers || snapshot.members.some((member) => !member.ready))) failures.push(`${snapshot.phase} room must have a full ready roster`);
	if (snapshot.phase === "finished" !== (snapshot.result !== void 0)) failures.push("result must exist exactly for a finished room");
	return failures;
}
//#endregion
export { validateLanRoomSnapshot as n, LanRoomError as r, LanRoom as t };
