import { LanMemberId } from "./room.js";
import { DoudizhuCardId, DoudizhuError, applyDoudizhuAction, createDoudizhuGame, doudizhuPrivateView, doudizhuPublicView, shuffleDoudizhuDeck } from "./doudizhu.js";
import { randomInt, randomUUID } from "node:crypto";
import { Service } from "@deepseek-ai/cordis";
import z from "@deepseek-ai/schemastery";
//#region lib/types/doudizhu-runtime/index.js
...
var DoudizhuGames = class extends Service {
	static inject = ["lanRooms", "lanRoomTransport"];
	static Config = z.object({
		roundsPerMatch: z.number().default(3),
		roundPauseMs: z.number().default(2e3),
		decisionTimeoutMs: z.number().default(5e4)
	});
	...
	constructor(ctx, config = {}) {
		...
		this.config = {
			...
			roundPauseMs: nonNegativeInteger(config.roundPauseMs ?? 2e3, "roundPauseMs"),
			decisionTimeoutMs: positiveInteger(config.decisionTimeoutMs ?? 5e4, "decisionTimeoutMs")
		};
		ctx.effect(() => ctx.lanRooms.onChanged(({ kind, room }) => {
			if (kind === "removed") {
				this.games.get(room.id)?.abort.abort(/* @__PURE__ */ new Error("room closed"));
				return;
			}
		...
		ctx.effect(() => async () => {
			...
			for (const entry of entries) entry.abort.abort(/* @__PURE__ */ new Error("DouDizhu runtime disposed"));
	...
	*/
	...
	start(room) {
		...
		if (durable?.finishedAt !== void 0 && durable.closedAt === void 0) {
		...
		} else state = recoveredState(recovered.checkpoint.state);
		const entry = {
			roomId: room.id,
			...
			totalScores: recovered === void 0 ? [
				0,
				0,
				0
			] : [...recovered.checkpoint.totalScores],
			roundResults: recovered === void 0 ? [] : recovered.checkpoint.roundResults.map((result) => recoveredResult(result)),
			decisionOutcomes: recovered === void 0 ? [] : recovered.checkpoint.decisionOutcomes.map((outcome) => ({
				historyIndex: outcome.historyIndex,
				afterStateVersion: outcome.afterStateVersion,
				seat: outcome.seat,
				source: outcome.source,
				...outcome.fallbackReason === void 0 ? {} : { fallbackReason: outcome.fallbackReason }
			})),
		...
		this.notify(entry);
		entry.done = (initialDeal === void 0 ? this.continueMatch(entry, room, recovered?.pendingDecision) : this.run(entry, room, initialDeal)).catch((error) => {
	...
	async continueMatch(entry, room, recoveredPending) {
		...
		while (entry.round <= this.config.roundsPerMatch) {
			let redeals = 0;
			this.notify(entry);
			if (pending === void 0 && entry.state.phase !== "finished") this.publish(entry, "running");
			...
			if (result === void 0) throw new Error("finished round has no result");
	...
	async step(entry, room, recoveredPending) {
		...
		if (member === void 0) throw new Error(`room has no member in seat ${seat}`);
		...
		const deadlineAt = recoveredPending === void 0 ? new Date(requestedAt.getTime() + this.config.decisionTimeoutMs) : new Date(recoveredPending.deadlineAt);
		...
		try {
			...
			action = doudizhuFallbackAction(view);
			source = "fallback";
			reason = error instanceof DecisionDeadlineExpired ? "timeout" : fallbackReason(this.ctx.lanRooms.get(entry.roomId), seat, timeout);
		}
		const nextState = applyDoudizhuAction({
			state: entry.state,
			seat,
			action
		});
		const outcome = {
			...
			seat,
			source,
			...reason === void 0 ? {} : { fallbackReason: reason }
		};
	...
	async createDurableMatch(entry, room, deal) {
		...
		const event = {
			...
			deal: entry.deal,
			deck: [...deal.deck],
			biddingStarter: deal.biddingStarter
		};
		const record = {
			schemaVersion: 1,
			game: "doudizhu",
			...
			expiresAt: new Date(at.getTime() + 864e5).toISOString(),
		...
		entry.recordRevision = (await persistence.create(record)).recordRevision;
	}
	async persistDealStarted(entry, deal) {
		await this.updateDurable(entry, (record, at) => {
			const event = {
				type: "deal-started",
				seq: nextEventSeq(record),
				at,
				round: entry.round,
				deal: entry.deal,
				deck: [...deal.deck],
	...
	async persistDecisionRequested(entry, requestId, seat, requestedAt, deadlineAt, recoveredPending) {
		await this.updateDurable(entry, (record, at) => {
			...
			if (recoveredPending !== void 0) events.push({
	...
	async persistActionCommitted(entry, requestId, action, nextState, outcome) {
		await this.updateDurable(entry, (record, at) => {
			const events = [...record.events];
			if (outcome.source === "fallback") events.push({
				type: "decision-abandoned",
				seq: nextEventSeq(events),
				at,
				requestId,
	...
	async updateDurable(entry, transform) {
		const persistence = this.ctx.get("lanGamePersistence");
		if (persistence === void 0) return;
		if (entry.recordRevision === void 0) throw new Error("durable LAN match has no record revision");
		entry.recordRevision = (await persistence.update(entry.roomId, {
			expectedRecordRevision: entry.recordRevision,
			expectedCheckpointStateVersion: entry.state.version
		}, (record) => transform(record, (/* @__PURE__ */ new Date()).toISOString()))).recordRevision;
	}
	...
	currentPersistedRoom(entry) {
		...
		if (room === void 0) throw new Error(`room ${JSON.stringify(entry.roomId)} disappeared before durable commit`);
	...
	recoverableRecord(room) {
		...
		if (record.finishedAt !== void 0 || record.closedAt !== void 0) return void 0;
		if (record.checkpoint.asOfSeq !== record.events.at(-1)?.seq) throw new Error(`durable match ${JSON.stringify(room.id)} checkpoint is not at the event-log tail`);
		if (JSON.stringify(record.config) !== JSON.stringify(this.config)) throw new Error(`durable match ${JSON.stringify(room.id)} config differs from the active runtime`);
		if (!sameRoomIdentity(record.room, room)) throw new Error(`durable match ${JSON.stringify(room.id)} does not match the restored room`);
		return record;
	}
	publish(entry, status) {
		...
		const snapshot = {
			...
			deal: entry.deal,
			totalScores: [...entry.totalScores],
			roundResults: entry.roundResults.map((result) => ({
				...result,
				scores: [...result.scores]
			})),
			...pendingSeat === void 0 ? {} : { decisionSeat: pendingSeat },
	...
	publishFailure(roomId, error) {
		this.ctx.lanRoomTransport.publishGameSnapshot(roomId, json({
			game: "doudizhu",
			status: "failed",
			error
		}));
...
function freshDeal() {
	const deck = shuffleDoudizhuDeck(() => randomInt(0, 4294967296) / 4294967296);
...
function persistedRoom(room, resumeTokens = {}) {
	return {
		...
		members: room.members.map((member) => ({
			...member,
			...resumeTokens[member.id] === void 0 ? {} : { resumeToken: resumeTokens[member.id] }
		})),
		...room.result === void 0 ? {} : { result: room.result }
	};
}
var DecisionDeadlineExpired = class extends Error {};
...
function sameRoomIdentity(record, room) {
	return record.id === room.id && record.code === room.code && record.revision === room.revision && record.phase === room.phase && record.coordinatorId === room.coordinatorId && record.maxMembers === room.maxMembers && record.members.length === room.members.length && record.members.every((member, index) => {
		const current = room.members[index];
		return current !== void 0 && member.id === current.id && member.seat === current.seat;
	});
}
function resumeTokens(transport, roomId) {
	return transport.resumeTokens?.(roomId) ?? {};
}
function nextEventSeq(recordOrEvents) {
	return (("events" in recordOrEvents ? recordOrEvents.events : recordOrEvents).at(-1)?.seq ?? -1) + 1;
}
...
function parseAction(value) {
	...
	if (value.type === "bid" && (value.score === 0 || value.score === 1 || value.score === 2 || value.score === 3)) return {
		type: "bid",
		score: value.score
	};
	if (value.type === "play" && Array.isArray(value.cards) && value.cards.every((card) => typeof card === "string")) return {
		type: "play",
		cards: value.cards.map((card) => DoudizhuCardId(card))
	};
...
*/
function doudizhuFallbackAction(view) {
	const pass = view.legalActions.find((action) => action.type === "pass");
	if (pass === void 0) return view.legalActions[0] ?? (() => {
		throw new Error("current seat has no legal fallback action");
	})();
	const previousSeat = view.lastPlay?.seat;
	const landlord = view.landlord;
	if (previousSeat !== void 0 && landlord !== void 0 && (view.yourSeat === landlord ? previousSeat !== landlord : previousSeat === landlord) && view.cardCounts[previousSeat] <= 2) return view.legalActions.find((action) => action.type !== "pass") ?? pass;
	return pass;
}
function compactResult(entry) {
	return JSON.stringify({
		rounds: entry.roundResults.length,
		scores: entry.totalScores
	});
}
function json(value) {
...
function abortableDelay(milliseconds, signal) {
	...
	return new Promise((resolve, reject) => {
		const onAbort = () => {
			clearTimeout(timer);
			reject(signal.reason instanceof Error ? signal.reason : /* @__PURE__ */ new Error("round pause aborted"));
		};
...
//#endregion
export { DoudizhuGames as default, doudizhuFallbackAction };
